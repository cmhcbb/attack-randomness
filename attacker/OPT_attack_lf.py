...
def opt_lf(x_in, y, net, steps, eps):
    x_in = x_in[0].cpu().numpy()
    y = y.item()
    amodel = PytorchModel(net, bounds=[0,1], num_classes=10)
    attack = OPT_attack_lf(amodel)
    x_adv=attack(x_in,y)
    x_adv=torch.FloatTensor(x_adv).cuda()
    x_adv.clamp_(0.0,1.0)
    return x_adv.unsqueeze(0)
...
class OPT_attack_lf(object):
    ...
    def attack_untargeted(self, x0, y0, alpha = 0.2, beta = 0.01, iterations = 1000):
        ...
        #print(model.predict_label(x0),y0)
        ...
        lbd, count, lbd_g2 = self.fine_grained_binary_search_local(model, x0, y0, best_theta)
        timestart = time.time()
        g1 = 1.0
        #theta, g2 = best_theta, g_theta
        theta, g2 = best_theta, lbd
        #print(g2)
        opt_count = 0
        stopping = 1e-8
        prev_obj = 100000
        for i in range(iterations):
            ...
            for _ in range(q):
                ...
                g1, count, lbd_hi = self.fine_grained_binary_search_local(model, x0, y0, ttt, initial_lbd = lbd_g2, tol=beta/500)
                opt_count += count
            ...
            for _ in range(15):
                ...
                new_g2, count, lbd_hi = self.fine_grained_binary_search_local(model, x0, y0, new_theta, initial_lbd = min_lbd, tol=beta/500)
                opt_count += count
                alpha = alpha * 2
                if new_g2 < min_g2:
                    min_theta = new_theta 
            ...
            if alpha < 1e-6:
                ...
                if (beta < 1e-8):
                    break
        g_theta, _ = self.fine_grained_binary_search_local_origianl(model, x0, y0, best_theta, initial_lbd = 1.0, tol=beta/500)
    ...
    def fine_grained_binary_search_local(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        ...
        temp_theta = np.clip(temp_theta - 0.2, 0.0, None)
        loss = np.sum(np.square(temp_theta))

        return loss, nquery, lbd_hi

    def fine_grained_binary_search_local_origianl(self, model, x0, y0, theta, initial_lbd = 1.0, tol=1e-5):
        nquery = 0
        lbd = initial_lbd
