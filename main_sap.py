...
parser.add_argument('--keep_prob', required=True, type=float, help='keep probability')
...
# Model
if opt.model == 'vgg':
    from models.vgg_sap import VGG
    net = nn.DataParallel(VGG('VGG16', nclass, opt.keep_prob, img_width=img_width).cuda())
    #net = VGG('VGG16', nclass, opt.keep_prob, img_width=img_width)
elif opt.model == 'aaron':
    from models.aaron_sap import Aaron
    net = nn.DataParallel(Aaron(nclass, opt.keep_prob).cuda())
else:
